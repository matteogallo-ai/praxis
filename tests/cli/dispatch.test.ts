...
describe("praxis dispatch", () => {
  test("`praxis version` prints praxis v0.10.0 and returns 0", async () => {
    const code = await run(["version"]);
    expect(code).toBe(0);
    expect(stdout()).toBe("praxis v0.10.0\n");
  });

  test("`praxis --version` short-form works", async () => {
    const code = await run(["--version"]);
    expect(code).toBe(0);
    expect(stdout()).toContain("praxis v0.10.0");
  });
  ...
  test("help mentions the new brief command", async () => {
    ...
    // v0.6 implements the full pipeline — help should advertise --full.
